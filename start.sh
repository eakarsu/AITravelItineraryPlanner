...
if [ "${NODE_ENV:-development}" != production ] && [ "${ENABLE_DEMO_CREDENTIAL_AUTOFILL:-true}" = true ]; then psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f "$PROJECT_DIR/backend/migrations/001_governed_travel.sql" >/dev/null; node "$PROJECT_DIR/backend/src/seed-governed.js"; fi
