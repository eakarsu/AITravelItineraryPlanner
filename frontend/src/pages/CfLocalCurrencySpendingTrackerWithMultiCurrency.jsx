...
export default function CfLocalCurrencySpendingTrackerWithMultiCurrency() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-local-currency-spending-tracker-with-multi-currency-conversion/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-local-currency-spending-tracker-with-multi-currency-conversion', project: 'AITravelItineraryPlanner' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-local-currency-spending-tracker-with-multi-currency-conversion/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Local currency spending tracker with multi-currency conversion</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AITravelItineraryPlanner</p>
      </div>
