...
export default function CfGroupTripCostSplitterWithSplitwiseStyle() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-group-trip-cost-splitter-with-splitwise-style-settlements/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-group-trip-cost-splitter-with-splitwise-style-settlements', project: 'AITravelItineraryPlanner' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-group-trip-cost-splitter-with-splitwise-style-settlements/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Group trip cost-splitter with Splitwise-style settlements</h1>
