...
export default function CfVisaDocumentAdvisorCheckingRequirementsByDestination() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-visa-document-advisor-checking-requirements-by-destination/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-visa-document-advisor-checking-requirements-by-destination', project: 'AITravelItineraryPlanner' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-visa-document-advisor-checking-requirements-by-destination/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Visa/document advisor checking requirements by destination</h1>
