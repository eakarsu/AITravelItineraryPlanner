...
export default function CfTravelCommunityFeaturesShareTripsFollowTravelers() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-travel-community-features-share-trips-follow-travelers/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-travel-community-features-share-trips-follow-travelers', project: 'AITravelItineraryPlanner' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-travel-community-features-share-trips-follow-travelers/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Travel community features (share trips, follow travelers)</h1>
