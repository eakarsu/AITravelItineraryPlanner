...
// Visa/document advisor checking requirements by destination
// Feature: visa-document-advisor
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Visa/document advisor checking requirements by destination.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'visa-document-advisor', payload, result);
    res.json({ success: true, feature: 'visa-document-advisor', ...result });
  } catch (err) {
    console.error('visa-document-advisor analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Visa/document advisor checking requirements by destination. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'visa-document-advisor', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'visa-document-advisor', title: 'Visa/document advisor checking requirements by destination', project: 'AITravelItineraryPlanner' });
});
