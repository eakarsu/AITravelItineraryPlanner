...
// Group trip cost-splitter with Splitwise-style settlements
// Feature: group-cost-splitter
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Group trip cost-splitter with Splitwise-style settlements.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'group-cost-splitter', payload, result);
    res.json({ success: true, feature: 'group-cost-splitter', ...result });
  } catch (err) {
    console.error('group-cost-splitter analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Group trip cost-splitter with Splitwise-style settlements. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'group-cost-splitter', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'group-cost-splitter', title: 'Group trip cost-splitter with Splitwise-style settlements', project: 'AITravelItineraryPlanner' });
});
