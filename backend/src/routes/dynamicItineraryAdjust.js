...
// Dynamic itinerary adjustment for real-time delay handling
// Feature: dynamic-itinerary-adjust
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Dynamic itinerary adjustment for real-time delay handling.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'dynamic-itinerary-adjust', payload, result);
    res.json({ success: true, feature: 'dynamic-itinerary-adjust', ...result });
  } catch (err) {
    console.error('dynamic-itinerary-adjust analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Dynamic itinerary adjustment for real-time delay handling. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'dynamic-itinerary-adjust', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'dynamic-itinerary-adjust', title: 'Dynamic itinerary adjustment for real-time delay handling', project: 'AITravelItineraryPlanner' });
});
