...
// Local currency spending tracker with multi-currency conversion
// Feature: multi-currency-tracker
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AITravelItineraryPlanner" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Local currency spending tracker with multi-currency conversion.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'multi-currency-tracker', payload, result);
    res.json({ success: true, feature: 'multi-currency-tracker', ...result });
  } catch (err) {
    console.error('multi-currency-tracker analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AITravelItineraryPlanner". Return strict JSON only.',
      `Feature: Local currency spending tracker with multi-currency conversion. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'multi-currency-tracker', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'multi-currency-tracker', title: 'Local currency spending tracker with multi-currency conversion', project: 'AITravelItineraryPlanner' });
});
